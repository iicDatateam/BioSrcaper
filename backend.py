...
def post_process(reports):

    reports['Pay']['Reports']['BasePayReport']['Statistics']
    df_rep = pd.DataFrame([reports['Pay']['Reports']['BasePayReport']['Statistics']])
    df_rep = df_rep.reset_index(drop=True)
    df_rep.index = df_rep.Count
    df_rep =df_rep[['Mean',	'StdDev','Skewness']]
    df_rep
    return(df_rep)
